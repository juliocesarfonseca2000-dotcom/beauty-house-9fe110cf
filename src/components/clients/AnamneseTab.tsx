    ...
    <div className="space-y-5">
      ...
      <div className="bh-card p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input label="Doenças crônicas" placeholder="Diabetes, hipertensão..." v={a.chronic_diseases} onChange={(v) => setA({ ...a, chronic_diseases: v })} />
          <Input label="Alergias" placeholder="Medicamentos, cosméticos..." v={a.allergies} onChange={(v) => setA({ ...a, allergies: v })} />
          <Input label="Medicamentos em uso" placeholder="Nome e dosagem" v={a.medications} onChange={(v) => setA({ ...a, medications: v })} />
          <Input label="Cirurgias recentes" placeholder="Tipo e data" v={a.recent_surgeries} onChange={(v) => setA({ ...a, recent_surgeries: v })} />
        </div>

        <div className="flex flex-wrap gap-x-6 gap-y-2 pt-1">
          <Check label="Gestante / Amamentando" v={a.pregnant_nursing} onChange={(v) => setA({ ...a, pregnant_nursing: v })} />
          <Check label="Marca-passo / Implante metálico" v={a.pacemaker_metal} onChange={(v) => setA({ ...a, pacemaker_metal: v })} />
          <Check label="Problema de coagulação" v={a.coagulation_issue} onChange={(v) => setA({ ...a, coagulation_issue: v })} />
        </div>

        <Textarea label="Condições raras / observações médicas" placeholder="Condições especiais, doenças raras..." rows={2} v={a.rare_conditions} onChange={(v) => setA({ ...a, rare_conditions: v })} />
        <Textarea label="Histórico estético (outras clínicas)" placeholder="Botox, preenchimento, outros procedimentos anteriores..." rows={2} v={a.aesthetic_history} onChange={(v) => setA({ ...a, aesthetic_history: v })} />
      </div>
      ...
      <div className="bh-card p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input label="Peso (kg)" placeholder="Ex: 60" v={m.weight_kg} onChange={(v) => setM({ ...m, weight_kg: v })} />
          <Input label="Cintura (cm)" placeholder="Ex: 72" v={m.waist_cm} onChange={(v) => setM({ ...m, waist_cm: v })} />
          <Input label="Quadril (cm)" placeholder="Ex: 96" v={m.hip_cm} onChange={(v) => setM({ ...m, hip_cm: v })} />
          <Input label="Abdômen (cm)" placeholder="Ex: 84" v={m.abdomen_cm} onChange={(v) => setM({ ...m, abdomen_cm: v })} />
          <Input label="Braço (cm)" placeholder="Ex: 30" v={m.arm_cm} onChange={(v) => setM({ ...m, arm_cm: v })} />
          <Input label="Coxa (cm)" placeholder="Ex: 58" v={m.thigh_cm} onChange={(v) => setM({ ...m, thigh_cm: v })} />
        </div>
      </div>
    </div>
  );
...
function Input({ label, placeholder, v, onChange }: { label: string; placeholder?: string; v: string; onChange: (v: string) => void }) {
  return (
    <div>
...
function Textarea({ label, placeholder, rows = 3, v, onChange }: { label: string; placeholder?: string; rows?: number; v: string; onChange: (v: string) => void }) {
  return (
    <div>
      <label className="block text-[11px] font-semibold text-text2 uppercase tracking-wider mb-1.5">{label}</label>
      <textarea
        ...
        placeholder={placeholder}
        onChange={(e) => onChange(e.target.value)}
        className="w-full px-3 py-2 rounded-lg border border-border text-sm bg-card resize-y"
      />
    </div>
  );
}
